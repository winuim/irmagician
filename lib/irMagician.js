...
function command(commandStr, completion, failure) {
    // console.log(commandStr.replace(/\r\n/, ""))
...
function led(param) {
    // console.log(`led ${param}`);
    return new Promise(resolve => {
        var commandStr = 'l,0\r\n'
        if (/on/i.test(param)) {
            commandStr = 'l,1\r\n'
        }
        command(commandStr, (data) => {
            console.log(data.toString())
            resolve()
        })
    })
...
function capture() {
    return new Promise((resolve, reject) => {
        var commandStr = "c\r\n"
        // console.log(commandStr.replace(/\r\n/, ""))
        this.port.write(commandStr, (error, bytesWritten) => {
            var logMessage = ""
            this.port.on("data", data => {
...
function setBank(bank, pos) {
    ...
    return new Promise((resolve, reject) => {
        if (pos != 0) {
            ...
            this.port.write(pattern, (error, result) => {
                if (error) {
                    console.log(error)
                    reject(error)
                    return
